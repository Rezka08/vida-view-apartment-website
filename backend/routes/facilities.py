...
@facilities_bp.route('/custom', methods=['POST'])
@jwt_required()
@role_required('owner', 'admin')
def create_custom_facility():
    """Create custom facility (Owner/Admin only)"""
    try:
        from flask_jwt_extended import get_jwt_identity
        ...
        facility_name = data['name'].strip()

        # Check if facility with same name already exists (case insensitive)
        existing_facility = Facility.query.filter(
            func.lower(Facility.name) == func.lower(facility_name),
            Facility.category == 'unit',
        ...
        if existing_facility:
            ...
            return jsonify({
                ...
                'facility': existing_facility.to_dict(),
                'is_new': False
            }), 200
